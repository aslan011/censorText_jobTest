...
test('can mask censored words', () => {
    const mockApp = new censorApp(["fruit", "veg"], 'fruit veg meat beans')
    expect(mockApp.censorMask()).toBe('f$$$t v$g meat beans');
});

test('does not remove required puncutation e.g. fullstops and commas', () => {
    const mockApp = new censorApp(["meow", "woof"], 'I have a cat named Meow, a dog name Woof. I love the dog a lot. He is larger than a small horse.')
    expect(mockApp.censorMask()).toBe('I have a cat named M$$w, a dog name W$$f. I love the dog a lot. He is larger than a small horse.');
});

test('should not mask puncutation that is within the word, should remove the symbol instead', () => {
    const mockApp = new censorApp(["don't", "jum"], 'Do not jump, don\'t jump, JUMP')
